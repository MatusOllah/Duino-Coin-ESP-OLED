...
namespace {
    ...
    void SelectNode() {
        ...
        while (input == "") {
            ...
            if (waitTime == 8) {
              #if defined(SERIAL_PRINTING)
                Serial.println("Using predefined node because telekom");
                input = R"({"ip":"45.147.7.140","name":"svko-node-2","port":1042,"region":"EU","server":"master","success":true})";
              #endif
            }
